...
def mainGeneration(articTquestF=True,complTembeddF=False,save_final=True,save_steps=False,display_df=False,step_pct=0.1,max_prompt=10000,dim=50,token_max_emb=7500,cara_max_emb=100):
    articTquestF = True
    complTembeddF = False
    model_list = [0] # [0,1,2]
    ...
    prompt_list = getDataToQuerryListLLM(max_prompt,articTquestF)
    ...
    for prompt in prompt_list:
        for model_n in model_list:
            for temperature_n in temperature_list :
                ...
                count = count + 1
    if save_final :
        saveDFcsv(df.set_index(set_index_key), save_path, filename_save+"_final",True)
    return df
